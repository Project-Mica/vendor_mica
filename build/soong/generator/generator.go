...
func init() {
	android.RegisterModuleType("mica_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = micaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = micaExpandVariables(ctx, dep_file)
	...
	cmd := micaExpandVariables(ctx, String(g.properties.Cmd))
