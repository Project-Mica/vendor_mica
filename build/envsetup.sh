...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^mica_") ; then
        MICA_BUILD=$(echo -n $1 | sed -e 's/^mica_//g')
    else
        MICA_BUILD=
    fi
    export MICA_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/mica/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch mica_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/mica-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.mica.device | grep -q "$MICA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MICA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function micaremote()
{
    ...
    local MICA_USER=$(git config --get review.review.micaos.org.username)
    if [ -z "$MICA_USER" ]
    then
        git remote add mica ssh://review.micaos.org:29418/$PFX$PROJECT
    else
        git remote add mica ssh://$MICA_USER@review.micaos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'mica' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.mica.device | grep -q "$MICA_BUILD");
    then
...
function micagerrit() {
    ...
    local user=`git config --get review.review.micaos.org.username`
...
function micarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      micarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.micaos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.mica.device | grep -q "$MICA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/mica/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MICA_FIXUP_COMMON_OUT ]; then
